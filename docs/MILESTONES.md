...
**Two items carried out of M6 rather than blocking it.** Both are recorded here so the `[x]` is not silently overclaiming:

1. **Nsight bank-conflict metric — deferred to M9 by design.** It needs `ncu`, and `docs/flash_attention_notes.md`'s change log already scheduled Nsight backfill for the M9 landing commit. The *prediction* attached to it (occupancy will not move) is independently confirmed by `ptxas`: 32 warps/SM for both kernels.
2. **`benchmarks/results/all.csv` still lacks the v2 rows**, and `docs/plots/*.png` predate v2. Until that lands, the 13.22× / 7.5× / 48% figures quoted above and in the README trace to `docs/ptxas_v1_vs_v2.md` and `theory/M6.md` §13.1 — not to a committed CSV row. That is a live exception to M5's portfolio hook (*"every claim in the README traces back to a row in `all.csv`"*) and should be closed by a `bench(m6)` commit before M10 polish.
...
- New plot row in `docs/plots/runtime_vs_N.png` showing v2 above v1. **⏳ Carried** — the sweep ran on Colab and v2 is comfortably above v1 at every shape, but the refreshed PNGs and `all.csv` were not brought back from the session. Regenerate with `scripts/bench.sh` and land as `bench(m6)`; see the second carried item under **Status**.
