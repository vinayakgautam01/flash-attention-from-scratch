...
**Status.** **Complete** on `5d07586` (2026-08-16), Colab Tesla T4 / nvcc 12.8. All 10 TODOs done. **Headline: 13.22× over v1 at `N = 2048, D = 64` (11.69 ms → 0.88 ms)**, with the full grid ranging 3.41×–13.22× and widening with `N`. The bar that actually mattered — the one `theory/M6.md` §13 named above the MILESTONES bar — is beating `attention_naive` at `N = 2048, D = 64`: v1 *lost* there (6.71 ms vs 11.69 ms, 0.57×), v2 wins by **7.6×** — and v2 now beats naive at **all ten shapes** (6.6×–21.9×), where v1 fell behind from `N = 1024` onward. The gap to `torch_ref` (cuBLAS SDPA, 0.426 ms) narrows from ~27× to ~2.1×, i.e. 3.6% → **48%** of torch's effective throughput. Correctness: `test_flash_fwd_v2` passes 25/25 (1 config guard + 8 edge + 10 `M6Grid` + 6 `M6Parity`), full `ctest` 79/79, pytest 74/74; `max_abs_err_vs_ref` is *identical* to v1's in all ten benchmark rows, which is the empirical form of the bit-parity claim. `ptxas`: **zero spills in either kernel** (v1 63 registers, v2 64 at both `D`), and warps/SM is 32 for both — so the reported "occupancy" is unchanged across a 13× speedup, exactly as `theory/M6.md` §7.2 predicted. Details in `docs/ptxas_v1_vs_v2.md` §3 and `theory/M6.md` §13.1, the latter also scoring the pre-registered 4–7× prediction (too conservative) and explaining why.

**One item carried out of M6 rather than blocking it**, recorded here so the `[x]` is not silently overclaiming: the **Nsight bank-conflict metric is deferred to M9 by design.** It needs `ncu`, and `docs/flash_attention_notes.md`'s change log already scheduled Nsight backfill for the M9 landing commit. The *prediction* attached to it (occupancy will not move) is independently confirmed by `ptxas`: 32 warps/SM for both kernels.

*(Previously also carried: the benchmark artifacts. **Closed** — `benchmarks/results/all.csv` is now the 50-row, five-variant sweep at `5d07586`, and `docs/plots/*.png` are regenerated with the v2 series. Every figure quoted above traces to a row in that file, restoring M5's portfolio hook.)*
...
- New plot row in `docs/plots/runtime_vs_N.png` showing v2 above v1. **✓ Passed** — `docs/plots/*.png` regenerated from the 50-row sweep; v2 sits below v1 (faster) at every shape on `runtime_vs_N.png`, and clears 1.0× everywhere on `speedup_vs_naive.png`, which v1 does not.
...
| M6  | FlashAttention v2 layout & occupancy       | L      | [x]    |
