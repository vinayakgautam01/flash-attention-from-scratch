...
> **What closing §§1, 2, 3, 4 and 11 bought (Colab Tesla T4, commit `5d07586`).** At \(N = 2048, D = 64\): **11.69 ms → 0.88 ms, a 13.2× speedup**. Across the grid the range is 3.4× (`N=128, D=32`) to 13.2×, widening with `N` — which is the expected shape, since the bank-conflict cost these items removed scales with the number of KV tiles.
>
