...
- [x] **M6 tile sizes.** `Br = Bc = 32`, unchanged from M4. `Br` is the only knob that changes HBM traffic, and doubling it to 64 would halve DRAM bytes — but DRAM is ~1.8% of v1's runtime, and `Br = 64` forfeits the second CTA per SM twice over: at `RowsPerThread = 2` it puts threads/CTA back at 1024 (one CTA by the thread limit), and even after raising `RowsPerThread` to 4 to restore a 512-thread block, smem is 32,896 B at `D = 64` — so two CTAs need 65,792 B against T4's 65,536, missing by 256 B. (At `D = 32` it would fit, so any future `Br = 64` path has to be `D`-specialized.) Optimize the 98%, not the 1.8%. Holding `Br` also makes v2's analytical HBM traffic *identical* to v1's, so any measured DRAM delta is unambiguously an efficiency effect. Likely to reverse on A10G (Ampere, ~100 KB smem/SM); see `theory/M6.md` §10.
- [x] **M6 launch config.** 512 threads/CTA (`block = (Bc, Br/2)`) with `RowsPerThread = 2`, declared to `ptxas` via `__launch_bounds__(512, 2)`. Reaches 2 CTAs/SM at both `D ∈ {32, 64}`. **Both** levers were needed, not either/or: 512 threads with v1's 37 KB smem still fits only one CTA, and 24 KB smem with 1024 threads is still one CTA by the thread limit.
- [x] **`D` becomes a template parameter in v2.** Required, not stylistic: the register-resident `Õ` array has length `L = D/Bc`, and if that is not compile-time, `ptxas` places it in *local* memory (physically DRAM) inside the hot loop. Costs an explicit `switch (D)` dispatch in the launch wrapper. This is the single easiest way to silently un-do M6 — output stays correct, speed does not. See `theory/M6.md` §6.2.
- [x] **Shared memory holds only `sQ`, `sK`, `sV` in v2.** `Õ`, `P`, `m` and `ℓ` are all register-resident. `P` travels lane-to-lane by `__shfl_sync` because row `i` is written and read entirely within one warp; `(m, ℓ)` are held redundantly by all 32 lanes because both warp reductions broadcast. Removes 12 KB and one of v1's three `__syncthreads()` per tile. Deleting the *buffer* is strictly safer than deleting the barrier while keeping it — with no shared array there is no cross-warp hazard to argue about.
- [x] **M6 test surface.** GoogleTest (`test_flash_fwd_v2`) as the primary gate, **plus** a Python-side v1-parity test in `tests/test_all_variants.py` — the first milestone to add both, because "drop-in replacement for v1" is a claim the pytest matrix should also be able to falsify. Parity bound is 1e-5 (tighter than the 5e-4 oracle tolerance): identical accumulation order means the two kernels should differ only by FMA contraction.
- [x] **v2 keeps FA-1 loop order.** `flash_fwd_v2` is *not* FlashAttention-2 the paper. Named prominently in the header, `theory/M6.md` §2, and `flash_attention_notes.md` §6, because the name invites exactly that misreading.
